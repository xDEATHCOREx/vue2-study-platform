 ...
 <mu-drawer  :open="open" :docked="docked" @close="closeMenu()">
  ...
  <router-link class="login-btn center-horizontal" :to="{path:'/login'}" v-if="!logIn">
      ...
      <mu-list @itemClick="docked ? '' : closeMenu()">
        <router-link :to="{path:'/'}">
        	<mu-list-item class="menu-item" title="首页" @click.native="closeMenu()">
        		 <mu-avatar slot="left" icon="home" :size="36" backgroundColor="purple600"/>
        	</mu-list-item>
        </router-link>
        <mu-divider />
        <router-link :to="{path:'/questions'}" >
          <mu-list-item class="menu-item" title="问答社区" @click.native="closeMenu()">
        ...
        <mu-divider />
          ...
          <mu-list-item class="menu-item" title="收藏" @click.native="closeMenu()">
               <mu-avatar slot="left" icon="star" :size="36" backgroundColor="yellow600"/>
          </mu-list-item>
        </router-link>
        <mu-divider />
         <router-link :to="target"  v-if="logIn">
          <mu-list-item class="menu-item" title="学习风格测试" @click.native="closeMenu()">
...
export default {
  data () {
    return {
      userInfo:{
        userPic:'http://18-newbee.top/img/blank.png'
      },
    ...
    isTested(){
      this.axios.get('style/getDimension.action')
        .then(res => {
          ...
          if(res.data.success){
            //已经过测试，改变路由到得分页面
            this.target = {path:'/score'}
          }else{
            this.target = {path:'/exam',query:{id:'1'}}
          }
        })
        .catch(err=> {
          console.log(err)
