...
<!--  <video  :src="detail.resource.resUrl"  width="100%" id = "container"  controls>播放器不可用，请更换浏览器重试。</video> -->
  <mu-card>
...
import evaluate from '../components/evaluate.vue'
    ...
    methods:{
      getResourceDetail(){
        //获取资源详细信息
        this.axios.get('/resource/detail.action?resourceId='+this.$route.params.resourceId)
        .then(res =>{
     ...
     beforeRouteLeave (to, from, next) {//可用于编写在离开此路由前的逻辑
      ...
      if(this.evaluated){  
        ...
        if(this.logIn){//这里检查登录状态若已登录才弹出评价！
          ...
          this.target = {
              path : to.path,
              query:{resourceId:to.params.resourceId}
           }
